...
if len(sys.argv)<4:
	...
	data = {'pilot_cores': ['24', '48', '96', '192'], 'queue_time_and_RP_overhead': [], 'exec': []}
	...
	bar_width = 0.5
	bar_l = [i+1 for i in range(len(data['pilot_cores']))]
	...
	plt.xticks(position, data['pilot_cores'])

	ax1.set_ylabel('Mean TTC (s)')
	ax1.set_xlabel('Pilot Cores')
