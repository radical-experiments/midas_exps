...
$131072 \times 131072 \times 8$ bytes, which is 128GBs.  

\item Scenario 2: Let's assume that the data are partitioned in blocks of 4096 and each task calculates 
a $4096 \times 4096$ matrix. The memory required at any given time, from cdist, would be 
$4096 \times 4096 \times 8 \times 24$ 
...
\item Scenario 3: Let's assume that the data are partitioned in blocks of 4096 and each task calculates
 a $4096 \times 131072$ matrix. The memory required at any given time, from cdist, would be 
 $4096 \times 132072 \times 8 \times 24$
...
\begin{figure}[H]
\centering
  \includegraphics[scale=0.6]{leaflet-finder/lfrp_sparkScipy_comet131k.pdf}
...
One extra experiment, that is undergoing, tries to compare the time that the RADICAL-Pilot
implementation is doing against the implementation with Dask that Andre did. Figure \ref{fig:compDaskRpComet1node}
shows that Dask does outperform RADICAL-Pilot. The question that rises is how many tasks does Dask
generate for the specific data points and how are these tasks scheduled. Each Task calculates a submatrix
of the final adjancency matrix. The submatrices size is $4096 \times 4096$ elements.

\begin{figure}[H]
\centering
  \includegraphics[scale=0.6]{leaflet-finder/compDaskRpComet1node.pdf}
  \caption{Comparing the execution time for calculating the distance matrices with Dask and RADICAL-Pilot. 
  One Comet node was used.}
  \label{fig:compDaskRpComet1node}
\end{figure}
