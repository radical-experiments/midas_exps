...
for atom in atoms:
    for style in styles:
        for coreCount in cores:
            for Try in tries:
                if style=='Br':
                    session='WraSpBrAlO_%dKn_%d_1024_%d'%(atom,coreCount,Try)
                    print session
                ...
                if style=='Col':
                    session='WraSpColAlO_%dKn_%d_1024_%d'%(atom,coreCount,Try)
                    print session
                    ...
                    ReducedCsv.loc[len(ReducedCsv)]=data
                if style=='Pcc':
                    session='WraSpPccAlO_%dKn_%d_1024_%d'%(atom,coreCount,Try)
                    print session
...
ReducedCsv.to_csv('Wrangler.csv')
